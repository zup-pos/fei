...
-- 模式切换（0=飞天, 1=移速, 2=穿墙, 3=透视, 4=重力, 5=跳跃）
local modeIndex = 0
local modeNames = { "fly", "speed", "noclip", "nightvision", "gravity", "jumppower" }
local modeDisplayNames = { "飞天", "移速", "穿墙", "透视", "重力", "跳跃" }
...
local gravityMin = -10000
local gravityMax = 10000
...
local function createGravityModeWindow()
    ...
    local minG = gravityMin
    local maxG = gravityMax
...
local function enableNightVision()
    ...
    nightVisionMaintainConnection = RunService.Heartbeat:Connect(function()
        if nightVisionEnabled then
            applyNightVision()
        end
    end)
    ...
    createMenu("UI菜单", {
        {
            ...
            callback = function(menu)
                ...
                local lines = {
                    ...
                    "- 主按钮：长按切换飞天/移速/穿墙/透视/重力/跳跃模式，单击开关当前模式",
                    "- 隐藏按钮：单击折叠UI，长按打开菜单",
                    "- 音量键控制：可在设置中开启/关闭",
                    "- 死亡自动关闭：可控制角色死后是否自动停用当前模式（仅影响飞天/移速）",
                    "- 穿墙：独立开关，重生后自动恢复，跳跃时新部件自动穿透",
                    "- 透视：独立开关，可调节亮度（0.01~100），重生后自动恢复",
                    "- 重力调节：独立开关，悬浮窗拖动滑块调节世界重力（-10000~10000），关闭窗口不关功能",
                    "- 跳跃力度：独立开关，悬浮窗拖动滑块调节角色跳跃力度（0~200），关闭窗口不关功能",
                    "",
                    "自定义屏幕尺寸：",
                    "如自动检测不准确，可手动设置屏幕宽高",
                    "",
        ...
        },
        {
            ...
            callback = function(menu)
                ...
                local lines = {
                    ...
                    "🔹 透视：独立开关，可调节亮度（0.01~100），重生后自动恢复",
                    "🔹 重力模式：独立开关，悬浮窗滑块实时调节世界重力（-10000~10000），关闭窗口不关功能",
        ...
        if holding then
            ...
            elseif modeIndex == 4 then
                showInputDialog(
                    ...
                    function(input)
                        local num = tonumber(input)
                        if num and num >= gravityMin and num <= gravityMax then
                            ...
                            if gravityWindow and gravityWindow:FindFirstChild("Frame") then
                                ...
                                if sliderBg then
                                    ...
                                    if knob then
                                        local percent = (workspace.Gravity - gravityMin) / (gravityMax - gravityMin)
                                        knob.Position = UDim2.new(percent, -6, 0, -4.5)
                                    end
                                end
                            end
                        ...
                        if num and num >= 0 and num <= 200 then
                            ...
                            if jumpPowerWindow and jumpPowerWindow:FindFirstChild("Frame") then
                                ...
                                if sliderBg then
                                    local knob = sliderBg:FindFirstChild("Knob")
                                    if knob then
                                        local percent = (currentJumpPower - 0) / 200
                                        knob.Position = UDim2.new(percent, -6, 0, -4.5)
                                    end
                                end
                            end
